...
class hr_payroll_ma(models.Model):
    ...
    def onchange_period_id(self):
        if self.partner_id and self.period_id:
            self.name = 'Paie '+ self.company_id.name + ' de la periode ' + self.period_id.name
    ...
    def draft_cb(self):
        ...
        for rec in self:
            if rec.move_id:
                raise ValidationError(u"Veuillez d'abord supprimer les écritures comptables associés %s", rec.move_id.name)
            ...
            for e in emp :
                ...
                if contract_ids and employe.company_id == rec.company_id:
                    ...
                    if rec.period_id.name == "01/2022":
                        ...
                        for leave in leaves:
                            number_days_not_working += leave.number_of_days

                    related_attendances = self.env['hr.attendance'].search(
                        [('employee_id', '=', employe.id), ('check_out', '<=', rec.date_end)])
                    line = {
                            'employee_id' : e.id,
                            ...
                            'attendance_ids': [(6, 0, related_attendances.ids)]
                    }
    ...
    def action_move_create(self):
        for rec in self:
            ...
            date = rec.date_end or time.strftime('%Y-%m-%d')
            ...
            #','.join(bulletin.id for bulletin in bulletins)
            bulletins_query_cond = '(' + ','.join(str(bulletin.id) for bulletin in bulletins) + ')'
            ...
            for e in move_lines:
                ...
                c +=  round(e[2]['credit'],2) if e[2]['credit'] else 0
                d +=  round(e[2]['debit'],2) if e[2]['debit'] else 0
...
class hr_payroll_ma_bulletin(models.Model):
    ...
    cumul_conges = fields.Float(compute='get_cumuls', string='Cumul conge', digits=(16, 2))
    cumul_chargefam = fields.Float(compute='get_cumuls', string='Cumul chargeFam', digits=(16, 2))
    worked_hours = fields.Float(compute='get_worked_hours', string='Heures de travail', digits=(16, 2))
    attendance_ids = fields.One2many(comodel_name='hr.attendance', inverse_name="bulletin_id", string="Pointages")

    def get_worked_hours(self):
        for bulletin in self:
            bulletin.worked_hours = sum(attendance.worked_hours for attendance in self.env['hr.attendance'].search([('bulletin_id', '=', bulletin.id)]))
    ...
    def get_bulletin_cumuls(self,mois, annee, employe):
        ...
        for res in self:
            ...
            period = acct_period.search([('name','=',v_period), ('company_id','=',res.employee_id.company_id.id)])
            ...
            if bulletin:
                ...
                if bul.frais_pro < 2500:
                    ...
                    cumuls['fp'] = 2500.0
                cumuls['conges_payes'] = bul.conges_payes
                cumuls['chargefam'] = bul.chargefam
            ...
            for res in self:
                ...
                for j in range(1,int(mois)+1,1):
                    ...
                    if valeur_mois:
                        ...
                        ''''''
                        somme_conges += valeur_mois['conges_payes']
                        somme_chargefam += valeur_mois['chargefam']
                ...
                res.cumul_conges = somme_conges
                res.cumul_chargefam = somme_chargefam
        ...
        for rec in self:
            ...
            if not bulletin.employee_contract_id.ir:
                ...
                '''if fraispro < dictionnaire.plafond:
                    ...
                    salaire_net_imposable = montant - (dictionnaire.plafond * (bulletin.working_days + bulletin.conges_payes)) / 26 - cotisations
                '''
                if montant <= 6500:
                    # fraispro = montant * dictionnaire.fraispro / 100
                    fraispro = montant * 0.35
                    if fraispro < 2500:
                        frais_pro_reelle = fraispro
                        salaire_net_imposable = montant - fraispro - cotisations
                    else:
                        ...
                        salaire_net_imposable = montant - (
                                    2500 * (bulletin.working_days + bulletin.conges_payes)) / 26 - cotisations

                else:
                    fraispro = montant * 0.25
                    if fraispro < 2917:
                        frais_pro_reelle = fraispro
                        salaire_net_imposable = montant - fraispro - cotisations
                    else:
                        frais_pro_reelle = dictionnaire.plafond
                        salaire_net_imposable = montant - (
                                    2917 * (bulletin.working_days + bulletin.conges_payes)) / 26 - cotisations
                ...
                base_calcul_ir = 0
                base_calcul_ir = bulletin.cumul_sni_n_1 + salaire_net_imposable
                print (bulletin.cumul_work_days)
                print (bulletin.cumul_work_days + bulletin.cumul_conges)
                ratio = (312/(bulletin.cumul_work_days + bulletin.cumul_conges) if (bulletin.cumul_work_days + bulletin.cumul_conges) else 1)
                print ('ratio', ratio)
                for tranche in liste:
                    if(base_calcul_ir * (312/(bulletin.cumul_work_days + bulletin.cumul_conges) if (bulletin.cumul_work_days + bulletin.cumul_conges) else 1) >= (tranche.debuttranche) and (base_calcul_ir * (312/(bulletin.cumul_work_days + bulletin.cumul_conges) if (bulletin.cumul_work_days + bulletin.cumul_conges) else 1) < (tranche.fintranche))):
                        taux = tranche.taux
                        somme = tranche.somme
                _logger.warning('SN1 %s %s %s %s' % (mt_rabattement, base_calcul_ir, ratio, base_calcul_ir *(26/(bulletin.cumul_work_days + bulletin.cumul_conges) if (bulletin.cumul_work_days + bulletin.cumul_conges) else 1)))
                _logger.warning('SN2 %s %s' % (taux, somme))
                print ('taux', taux)
                print ('somme', somme)
                ir_brute = (base_calcul_ir * (312/(bulletin.cumul_work_days + bulletin.cumul_conges) if (bulletin.cumul_work_days + bulletin.cumul_conges) else 1) * taux / 100) - somme
                ...
                ir_brute *= ((bulletin.cumul_work_days + bulletin.cumul_conges)/312)
                print ('ir_brute3', ir_brute)
                _logger.warning('IR3 %s' % (ir_brute))
                ir_brute -= (bulletin.cumul_chargefam * dictionnaire.charge)
                ir_brute -= bulletin.cumul_igr_n_1
                print ('ir_brute4', ir_brute)
    ...
    def compute_all_lines(self):
        ...
        for rec in self:
            ...
            working_days = bulletin.working_days + bulletin.conges_payes
            ...
            conges_payes = bulletin.conges_payes
            ...
            if salaire_base :
                absence += salaire_base - (salaire_base * (working_days / 26))

                salaire_base_line = {
                    'name' : 'Salaire de base', 'id_bulletin' : id_bulletin, 'type' : 'brute','base' : round(salaire_base,2),
                    'rate_employee' : round((working_days / 26) * 100,2), 'subtotal_employee':round(salaire_base * (working_days / 26),2),'deductible' : False,
                   }
                salaire_brute += salaire_base * (working_days / 26)
            ...
            '''if conges_payes:
                ...
                leave_line = {
                    'name' : 'Jours congés payés', 'id_bulletin' : id_bulletin, 'type' : 'brute','base' : round(salaire_base,2),
                    'rate_employee' : round((conges_payes / 26) * 100,2), 'subtotal_employee':round(salaire_base * ( conges_payes / 26),2),'deductible' : False,
                   }
                salaire_brute += salaire_base * ( conges_payes / 26)

                self.env['hr.payroll_ma.bulletin.line'].create(leave_line)'''
                ...
                if(rubrique['categorie'] == 'majoration'):
                    ...
                    if rubrique['ir']:
                        if rubrique['plafond_ir'] == 0:ir += montant
                            ...
                            ir += ecart * (working_days / 26 if rubrique['absence'] else 1) if ecart > 0 else 0
                        elif montant > rubrique['plafond_ir']:
                            '''if rubrique['plafond_ir']:
                                ir += montant - rubrique['plafond_ir']
                            else:
                                ir += montant'''
                            ir += montant - rubrique['plafond_ir'] * (working_days / 26 if rubrique['absence'] else 1)
            ...
            if bulletin.employee_id.affilie:
                for cot in cotisations :
                        ...
                        for imposition in imposition_ids:
                            if imposition.plafonne:
                                if imposition.plafond_mt < rubrique_dict['montant']:
                                    base_cotisation += (rubrique_dict['montant'] * (working_days / 26 if rubrique.absence else 1) - imposition.plafond_mt * (working_days / 26 if rubrique.absence else 1))
                            else:
                                base_cotisation += rubrique_dict['montant'] * (working_days / 26 if rubrique.absence else 1)
                    ...
                    if cot.absence_pp:
                        absence_ratio_pp = working_days / 26
                    if cot.absence_ps:
                        absence_ratio_ps = working_days / 26
            ...
            salaire_net = round(salaire_brute,2) - round(res['ir_net'],2) - round(cotisations_employee,2)

            salaire_net_a_payer = salaire_brute - deduction - res['ir_net'] - cotisations_employee
            salaire_net_a_payer = round(salaire_net_a_payer, 2)
            ...
            if bulletin.employee_contract_id.taxe_solidarite == 'fixe':
                ...
                pret = self.env['hr.payroll_ma.bulletin.line'].search([('id_bulletin', '=', id_bulletin), ('name', '=', 'PRET AID')], limit=1)
                exlude_amount += pret.subtotal_employee if pret else 0
                ...
                salaire_net_css = salaire_brute - (rappel.subtotal_employee if rappel else 0) - (pret.subtotal_employee if pret else 0 ) - cotisations_employee - ir_css['ir_net']
            ...
            rec.chargefam = bulletin.employee_id.chargefam
