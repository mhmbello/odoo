...
class hr_employee(models.Model):
    ...
    numero_categorie_cimr = fields.Char(
        string='N° Catégorie CIMR',
    )
    date_affiliation_cimr = fields.Date(
    ...
    def _check_param_logement_social(self):
        if self.type_logement == 'social':
            ...
            if self.superficie_logement > dictionnaire['superficie_max_logement_social']:
                raise UserError(('Attention'), (u"La superficie indiquée n'est pas conforme aux normes du logement social !!!"))

            # On vérifie le prix d'acquisition
            if self.prix_acquisition_logement > dictionnaire['prix_achat_max_logement_social']:
                raise UserError(('Attention'), (u"Le prix d'acquisition n'est pas conforme aux normes du logement social !!!"))
...
class hr_contract(models.Model):
    ...
    brut_salary = fields.Float(string="Brut")
    ...
    def _check_unicite_contrat(self):
        ...
        if len(contrat_ids) > 1:
            raise UserError(('Attention'), (u'Plusieurs contrats actifs pour cet employé!!!'))
    ...
    def net_to_brute(self):
        for rec in self:
            ...
            rec.write({'brut_salary': round(salaire_brute,2) })
            return True
